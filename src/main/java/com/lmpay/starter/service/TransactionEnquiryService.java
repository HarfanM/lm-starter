...
public class TransactionEnquiryService {
    ...
    public String getTransactionEnquiry(TransactionEnquiryDto transactionEnquiryDto) {
        try {
            PartnerTransaction sampleId = partnerTransactionRepository.findById(2).get();
            String transactionNumber = transactionEnquiryDto.getLMTransactionRefNo();
            log.info("Initializing transaction enquiry for");
            PartnerTransaction partnerTransaction = partnerTransactionRepository.findByTransactionNo(transactionNumber);
            if (partnerTransaction != null && partnerTransaction.getPartnerName().equals(BankType.FederalBank)) {
            ...
            else if(partnerTransaction != null && partnerTransaction.getPartnerName().equals("HelloPaisa"))
                {
            ...
            try {
                ...
                if (responseBody != null) {
                    ...
                    try {
                        ...
                        lmResponse.setLMResponseMessageCode(dcmResponseNode.get("responseCode").asText());
